...
MSVehicle::getSafeFollowSpeed(const std::pair<const MSVehicle*, double> leaderInfo,
	...
	////////////////////////////////
	double CVvsafeLeader=-1;
	double cvleaderSpeed=this->getCVLeaderSpeed();
	//////////////////////////

	//std::cout << ">>>>>>>>>>>>>>   Received Leader Speed = " << myCVLeaderSpeed<< ",  "<< this->getCVLeaderSpeed() << "\n";
//	followSpeed(const MSVehicle* const veh, double speed, double gap, double predSpeed, double predMaxDecel, const MSVehicle* const pred)
...
//		std::cout << ">>>>>>>>>>>>>>   " << SIMTIME << " cvleaderSpeed: " << cvleaderSpeed << "\n";
		if (cvleaderSpeed >=0.0 )
		{   // Sumo runs with NS3 and received the lreader speed info
			CVvsafeLeader = cfModel.followSpeed(this, getSpeed(), leaderInfo.second, cvleaderSpeed, leaderInfo.first->getCurrentApparentDecel(), leaderInfo.first);

			if  (CVvsafeLeader > vsafeLeader)
			{
				std::cout << "\nPotentialCollision:  " << SIMTIME << "  " << this->getID() << " " << leaderInfo.first->getID() <<
						" " << vsafeLeader  << " " << CVvsafeLeader<<  " " <<  leaderInfo.first->getSpeed() << "  " << leaderInfo.second <<  "  " << cvleaderSpeed << "\n";


//				std::cout << "\n>>>>>>>>>>>>>>   " << SIMTIME << " Potential accident: " << this->getID() << " may hit  " << leaderInfo.first->getID() << "\n";
//				std::cout << ">>>>>>>>>>>>>>   Computed Speed Error = " << CVvsafeLeader-vsafeLeader <<  ",  Gap=" << leaderInfo.second  << "\n";
//				std::cout << ">>>>>>>>>>>>>>   Received Leader Speed   = " << cvleaderSpeed << "\n";
//				std::cout << ">>>>>>>>>>>>>>   Actual Leader Speed     = " << leaderInfo.first->getSpeed() << "\n";
//				std::cout << ">>>>>>>>>>>>>>   Speed based on Actual Leader Speed  = " << vsafeLeader << "\n";
//				std::cout << ">>>>>>>>>>>>>>   Speed based on CV Leader Speed      = " << CVvsafeLeader  << "\n";
				}
			vsafeLeader = CVvsafeLeader;
		}
